...
CREATE TABLE `s_goods`  (
                            ...
                            `marketPrice` double NOT NULL COMMENT '价格',
...
CREATE TABLE `s_order`  (
                            ...
                            `repeatPv` double NOT NULL COMMENT '重消 PV。必填  根据消费类型，取基础信息表中相应 PV 换算\r\n公式得出',
                            `basePv` double NOT NULL COMMENT '基本 PV。必填  根据消费类型，取基础信息表中相应 PV 换算\r\n公式得出',
                            `serviceFee` double NOT NULL COMMENT ' 服务费。必填 根据消费类型、消费金额得出',
                            `shipFre` double NOT NULL COMMENT '快递费。必填  根据收获地址得出',
                            `tax` double NOT NULL COMMENT '税费。必填  根据消费类型、消费金额得出',
...
CREATE TABLE `s_orderList`  (
                                ...
                                `eachPrice` double NOT NULL COMMENT '单价。必填',
                                `sumPrice` double NOT NULL COMMENT '总价。必填',
                                `discount` double NULL DEFAULT NULL COMMENT '折扣。可选',
...
CREATE TABLE `s_userCash`  (
                               ...
                               `creditedMoney` double NULL DEFAULT NULL COMMENT '到账金额。系统生成',
...
CREATE TABLE `s_userRefer`  (
                                ...
                                `bonusRate` double NOT NULL COMMENT '取自基础数据表相应字段',
                                `bonusPv` double NOT NULL COMMENT '取自基础数据表相应字段',
                                `del` int(0) NULL DEFAULT 0,
                                PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_bin ROW_FORMAT = Dynamic;
